...
> **⚠️ STALE — snapshot pre-`Chat` entity.** This doc's core premise ("no Chat/Session entity, one conversation per character forever") is **no longer true**. A `Chat` model was added in commit `ad26373` ("feat(rp): independent per-chat persona / storylines (B8)") — see `src/backend/db/models.py:146` and `claude.md`'s "Two-layer conversational state" section. Every finding below that assumes character_id-only scoping needs to be re-verified against the current schema before acting on it. Kept as historical record of the pre-Chat-entity analysis, not as current documentation.
