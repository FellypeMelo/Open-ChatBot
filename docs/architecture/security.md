...
Currently, there is no session authentication (JWT/OAuth) in the API layer because the system operates in a single-tenant local environment (binding to localhost loopback `127.0.0.1`). If deployment to external networks is required, authentication must be added to the FastAPI layer. Refer to [auth.md](file:///G:/Programas/Open-ChatBot/docs/api/auth.md) for recommendation guidelines.
