...
*   **Chat History Clearing:** Clearing conversations via the UI invokes [clear_chat_history](file:///G:/Programas/Open-ChatBot/src/backend/api/chat.py#L197) which explicitly executes SQL `DELETE` queries on `message_nodes` and `journal_entries` for the selected character, resetting state metadata immediately.
