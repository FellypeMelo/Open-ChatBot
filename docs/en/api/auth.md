...
* **Auto-user Provisioning:** The backend dynamically registers/manages a single active user in SQLite database (`chatbot.db`) through the `/users/me` endpoint. See [users.py](../../../src/backend/api/users.py) for implementation details.
