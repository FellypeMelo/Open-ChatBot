...
## 1. Executive Summary
Open-ChatBot is a local-first, single-user AI interaction engine for high-fidelity character simulation. It is not a commercial product or a platform aimed at a market segment — it is a personal engineering project providing persistent, context-aware, and emotionally reactive AI characters that run entirely on the user's own machine, against the user's own model weights.
