...
*   **Auto-Orchestration:** Upon launch, the backend reads configuration from `models_config.json` and invokes the [LlamaServerRunner](file:///G:/Programas/Open-ChatBot/src/backend/core/engine/runner.py) to spin up a single unified `llama-server.exe` instance.
...
*   **Lifespan Shutdown Hook:** When the FastAPI instance is stopped, it calls `runner.stop_all()` to terminate the running background `llama-server.exe` instance, avoiding zombie processes. See [main.py](file:///G:/Programas/Open-ChatBot/src/backend/main.py#L52-L56).

## 2. Health Monitoring & Verification
*   **Startup Verification:** During application startup, the server performs embedding and inference health checks via [LlamaClient](file:///G:/Programas/Open-ChatBot/src/backend/core/engine/llm.py). 
...
*   **SQLite Vacuuming:** On startup, the backend calls `vacuum_db()` which executes the SQL command `VACUUM` asynchronously. This reduces disk fragmentation and reclaims unused database space, ensuring long-term filesystem health. See [database.py](file:///G:/Programas/Open-ChatBot/src/backend/db/database.py#L16-L22).
*   **Vector Database Bit Width:** To manage performance constraints on local consumer hardware, the TurboQuant vector database index uses a **4-bit quantization width** (`bit_width=4`) to store embeddings, reducing RAM utilization and speeding up cosine similarity searches. See [vector_store.py](file:///G:/Programas/Open-ChatBot/src/backend/core/memory/vector_store.py#L66-L95).
