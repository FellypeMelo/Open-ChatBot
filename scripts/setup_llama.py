...
def detect_hardware() -> str:
    ...
    try:
        ...
        if res.returncode == 0:
            print(
                "  [+] NVIDIA GPU detected via nvidia-smi. CUDA option is recommended."
            )
            return "cuda"
    ...
    if sys.platform == "win32":
        try:
            ...
            if "amd" in gpu_info or "radeon" in gpu_info:
                print(
                    "  [+] AMD GPU detected. HIP/ROCm or Vulkan option is recommended."
                )
                return "hip"
        except Exception:
            pass
    ...
    print(
        "  [-] No specialized GPU accelerator auto-detected (or driver CLI tools not in PATH)."
    )
    print(
        "  [!] Vulkan (Cross-vendor GPU) or AVX2 (CPU fallback) are the safest defaults."
    )
    return "vulkan"
            ...
            with open(dest_path, "wb") as f:
                while True:
                    ...
                    if total_size > 0:
                        ...
                        mb_downloaded = downloaded / (1024 * 1024)
                        print(
                            f"\r  Progress: {mb_downloaded:.1f} MB downloaded",
                            end="",
                            flush=True,
                        )
            print("\n  [+] Download complete.")
...
def main() -> None:
    ...
    if sys.platform != "win32":
        ...
        print(
            "[!] For Linux/macOS, please compile llama.cpp manually and place files in llama_bin/."
        )
        sys.exit(1)
    ...
    print(
        "  2) Vulkan          (Recommended for modern GPUs of any brand - no SDK needed)"
    )
    ...
    choice_map = {"1": "cuda", "2": "vulkan", "3": "sycl", "4": "hip", "5": "avx2"}
    ...
    try:
        user_choice = input(
            f"\nSelect target platform [1-5] (default {rec_num} - {recommended}): "
        ).strip()
        if not user_choice:
            user_choice = rec_num
    ...
    if gguf_files:
        ...
        try:
            ...
            if not download_opt or download_opt == "y":
                ...
                print(
                    "  [*] Skipping model download. Please place a GGUF model file in the models/ directory manually."
                )
        except KeyboardInterrupt:
