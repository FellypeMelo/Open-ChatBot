...
def test_chat_with_config(client, db_session):
    ...
    with patch(
        ...
        response = client.post(
            "/chat", json={"message": "hi", "character_id": 1, "config": config}
        )
...
def test_chat_stream_with_config(client, db_session):
    ...
    with patch(
        ...
        response = client.post(
            "/chat/stream", json={"message": "hi", "character_id": 1, "config": config}
        )
...
def test_update_character_state(client, db_session):
    ...
    update_payload = {
        "location": "Kitchen",
        "stats": {"energy": 90, "hunger": 10, "relationship_score": 75},
    }
...
def test_settings_endpoints(client):
    with (
        patch("src.backend.api.settings.runner.get_status") as mock_status,
...
def test_tag_endpoints_404(client):
    # PUT /tags/99999 -> 404
    response = client.put(
        "/tags/99999", json={"label": "Nonexistent", "instruction": ""}
    )
    assert response.status_code == 404
...
def test_chat_with_action_ids(client, db_session):
    ...
    state = AgentState(
        character_id=4,
        stats={"energy": 50, "hunger": 20, "relationship": {"score": 50}},
    )
    db_session.add(state)
    db_session.commit()
...
def test_chat_stream_with_action_ids(client, db_session):
    ...
    with patch(
        ...
        resp = client.post(
            "/chat/stream", json={"character_id": 5, "action_id": "tease"}
        )
        assert resp.status_code == 200
...
async def test_run_consciousness_layer_exception():
    ...
    with patch("src.backend.api.chat.logger") as mock_logger:
        with patch(
            "src.backend.api.chat.vector_store.add_memory",
...
def test_lore_endpoints(client, db_session):
    with patch(
        "src.backend.api.lore.vector_store.add_lore", new_callable=AsyncMock
    ) as mock_add_lore:
