...
def test_e2e_database_migration():
    ...
    cursor.execute("INSERT INTO users (id, name, is_active) VALUES (1, 'LegacyUser', 1)")
    conn.commit()
    conn.close()
    ...
    with patch("src.backend.db.database.engine", test_engine):
        ...
        assert "persona_description" in user_columns, "Migration failed: persona_description missing"
        ...
        assert "active_summary" in agent_columns, "Migration failed: active_summary missing"
        ...
        assert "short_description" in char_columns, "Migration failed: short_description missing"
        assert "persona_prompt" in char_columns, "Migration failed: persona_prompt missing"
        
        # 6. Verify legacy data was preserved and we can write to the new columns using raw SQL
        with test_engine.begin() as db_conn:
            # Check legacy data
            legacy_user = db_conn.execute(text("SELECT name FROM users WHERE id=1")).scalar()
            assert legacy_user == 'LegacyUser', "Data loss during migration!"
            
            # Write to new column
            db_conn.execute(text("UPDATE users SET persona_description = 'Updated Persona' WHERE id=1"))
            updated_desc = db_conn.execute(text("SELECT persona_description FROM users WHERE id=1")).scalar()
