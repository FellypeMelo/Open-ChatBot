...
def test_evolve_character_edge_cases():
    ...
    def mock_query(model):
        ...
        if model == Character:
            ...
            q.filter.return_value.with_for_update.return_value.first.return_value = agent_state
        return q
...
def test_get_behavioral_modifiers():
    ...
    assert "polite but guarded" in get_behavioral_modifiers({"relationship": {"score": 40}})
    assert "warm, open" in get_behavioral_modifiers({"relationship": {"score": 70}})
    assert "deeply affectionate" in get_behavioral_modifiers({"relationship": {"score": 90}})
