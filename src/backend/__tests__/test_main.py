...
async def test_lifespan_non_testing_mode():
    # Simulate production mode by turning off the centralized TESTING flag.
    mock_runner = MagicMock()
    ...
    # is_testing is False in this test (TESTING patched off), so the lifespan
    # really would call init_db()/vacuum_db()/seed_default_presets() against the
    # real chatbot.db -- mock all three so this test can never touch it.
    with (
