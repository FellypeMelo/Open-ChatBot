...
async def test_vector_store_disk_loading(tmp_path):
    ...
    with patch("turbovec.langchain.TurboQuantVectorStore.load", side_effect=Exception("Load error")):
    ...
    with patch("turbovec.langchain.TurboQuantVectorStore.load", return_value=mock_store_loaded):
        store = VectorStore(llm_client=mock_llm, path=str(test_path))
...
async def test_vector_store_lore_operations(tmp_path):
    ...
    mock_lore_store.asimilarity_search_with_score = AsyncMock(return_value=[(mock_doc, 0.9)])
...
async def test_vector_store_memory_operations(tmp_path):
    ...
    mock_mem_store.aadd_texts.assert_called_once_with(["hello"], metadatas=[{"test": True}])
    ...
    mock_mem_store.asimilarity_search_with_score = AsyncMock(return_value=[(mock_doc, 0.85)])
    
    res = await store.query_memory("query", n_results=3, metadata_filter={"user": "123"})
    assert res == {"documents": [["mem_content"]]}
    mock_mem_store.asimilarity_search_with_score.assert_called_once_with("query", k=3, filter={"user": "123"})
