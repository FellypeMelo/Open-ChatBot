        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Tokenize endpoint failed with {response.status_code}. Fallback to word count * 1.3")
                    return int(len(text.split()) * 1.3)
        except Exception as e:
            logger.error(f"Error calling tokenize endpoint: {e}. Fallback to word count * 1.3")
            return int(len(text.split()) * 1.3)
