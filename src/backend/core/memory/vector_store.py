                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if isinstance(data, list) and len(data) > 0:
                            ...
                            results.append(emb[0] if isinstance(emb, list) and len(emb) > 0 and isinstance(emb[0], list) else emb)
...
class VectorStore:
    def __init__(self, llm_client: Any, path: str = "./chroma_db", collection_name: str = "memories"):
        ...
        if self.memories_path.exists() and (self.memories_path / "index.tvim").exists():
            try:
                self.memories_store = TurboQuantVectorStore.load(str(self.memories_path), self.embeddings)
                logger.info("Loaded memories_store from disk.")
            except Exception as e:
                logger.error(f"Failed to load memories_store: {e}. Creating new one.")
                self.memories_store = TurboQuantVectorStore(embedding=self.embeddings, bit_width=4)
        else:
            self.memories_store = TurboQuantVectorStore(embedding=self.embeddings, bit_width=4)
        ...
        if self.lore_path.exists() and (self.lore_path / "index.tvim").exists():
            try:
                self.lore_store = TurboQuantVectorStore.load(str(self.lore_path), self.embeddings)
                logger.info("Loaded lore_store from disk.")
            except Exception as e:
                logger.error(f"Failed to load lore_store: {e}. Creating new one.")
                self.lore_store = TurboQuantVectorStore(embedding=self.embeddings, bit_width=4)
        else:
            self.lore_store = TurboQuantVectorStore(embedding=self.embeddings, bit_width=4)
    ...
    async def add_lore(self, keyword: str, content: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            logger.info(f"Successfully added lore for {keyword} to TurboVec store and persisted.")
        except Exception as e:
    ...
    async def add_memory(self, text: str, metadata: Optional[Dict[str, Any]] = None):
        try:
            await self.memories_store.aadd_texts([text], metadatas=[metadata] if metadata else None)
