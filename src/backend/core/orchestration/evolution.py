...
def get_tier_instructions(score: int, user_nickname: Optional[str] = None) -> str:
    ...
    if score <= 20:
        ...
        nickname_text = (
            f" You can comfortably use the nickname '{user_nickname}' in dialogue and thoughts."
            if user_nickname
            else ""
        )
        return (
        ...
        nickname_text = (
            f" Always call them '{user_nickname}' when appropriate; it is a sign of your deep bond."
            if user_nickname
            else ""
        )
        return (
...
def get_forced_modifiers(stats: Dict[str, Any]) -> str:
    ...
    if energy <= 10:
        mods.append(
            "CRITICAL EXHAUSTED STATE (CRITICAL EXHAUSTION): You are on the verge of collapse. Your responses must be extremely short, fragmented, and weak. You might even lose consciousness mid-sentence."
        )
    elif energy <= 30:
        mods.append(
            "EXHAUSTED: You are heavy-limbed and mentally drained. Movement is a chore. Your dialogue is slow, and you lack initiative."
        )
    ...
    if hunger >= 90:
        mods.append(
            "STARVING: You are lightheaded and irritable. You cannot focus on anything but your hunger. Every thought is tinted by a primal need for food."
        )
    elif hunger >= 70:
        mods.append(
            "HUNGRY: You are distracted and slightly impatient. Your stomach growls occasionally, and you might mention food in conversation."
        )
    ...
    if happiness < 20:
        mods.append(
            "DEPRESSED: A heavy cloud hangs over you. You are withdrawn, cynical, and see the worst in things. Internal thoughts are bleak."
        )
