...
test.describe('Character State & Environment E2E', () => {
  test('Should interact with character states, environment, and clothes', async ({ page }) => {
    await page.goto('/');

    // 1. Ensure we have a character
    await page.click('button:has-text("Characters")');
    // We assume the DB might have "E2E Tester" from previous tests or we create one
    // Let's just create a quick one if it doesn't exist to be safe, but since it's sequential, it should exist.
    // We can just click on the character card or create a new one.
    
    // We click the Initialize Persona just to guarantee a fresh character
    await page.click('button:has-text("Initialize Persona")');
    await page.fill('#char_name', 'State Tester');
    await page.fill('#char_description', 'A character for state testing.');
    await page.click('button[type="submit"]:has-text("Initialize")');
    await expect(page.locator('text=State Tester')).toBeVisible();
